<?php
// bookings_calendar.php  (เวอร์ชันแสดงชื่อโปรไฟล์ลูกค้า)
session_start();
...
// เตรียม payload ให้ JS (เพิ่ม cust_name)
$payload = [];
...
/* ===== THEME COLORS ONLY (โครงสร้างเดิม ไม่แตะ) ===== */
/* ===== ปรับธีม CY Arena สดใส ===== */
:root {
  --bg: #f5f7fb;
  --card: #ffffff;
  --ink: #0f172a;
...
/* เส้นแบ่งใต้ชื่อวัน + พื้นหลังชื่อวัน */
.calendar-grid {
  display: grid;
...
.cal-cell.today {
  border-color: #3b82f6;
  box-shadow: inset 0 0 0 2px #3b82f6, 0 6px 18px rgba(37, 99, 235, 0.15);
  background: #e0f2fe;
}
